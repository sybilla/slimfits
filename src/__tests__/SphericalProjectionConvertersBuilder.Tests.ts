...
describe("SphericalProjectionConvertersBuilder tests.", () => {
    ...
    it("ZenithalEquidistantProjectionConverter \"ARC\" projection converter convert/convertBack tests.", () => {
        let src = new ArrayBufferFile(fs.readFileSync('data/1904-66_ARC.fits').buffer);
        return src.initialize()
            .then(_ => FitsReader.readFitsAsync(src))
            .then(hdus => {
                expect(hdus.length).toEqual(1);
                let header = hdus[0].header;
                let builder = new SphericalProjectionConvertersBuilder();
    ...
    it("SlantOrtographicProjectionConverter \"SIN\" projection converter convert/convertBack tests.", () => {
        let src = new ArrayBufferFile(fs.readFileSync('data/1904-66_SIN.fits').buffer);
        return src.initialize()
            .then(_ => FitsReader.readFitsAsync(src))
            .then(hdus => {
                expect(hdus.length).toEqual(1);
    ...
    it("StereographicProjectionConverter \"STG\" projection converter convert/convertBack tests.", () => {
        let src = new ArrayBufferFile(fs.readFileSync('data/1904-66_STG.fits').buffer);
        return src.initialize()
            .then(_ => FitsReader.readFitsAsync(src))
            .then(hdus => {
                expect(hdus.length).toEqual(1);
    ...
    it("GnomonicProjectionConverter \"TAN\" projection converter convert/convertBack tests.", () => {
        let src = new ArrayBufferFile(fs.readFileSync('data/1904-66_TAN.fits').buffer);
        return src.initialize()
            .then(_ => FitsReader.readFitsAsync(src))
            .then(hdus => {
                expect(hdus.length).toEqual(1);
    ...
    it("GnomonicProjectionConverter \"ZEA\" projection converter convert/convertBack tests.", () => {
        let src = new ArrayBufferFile(fs.readFileSync('data/1904-66_ZEA.fits').buffer);
        return src.initialize()
            .then(_ => FitsReader.readFitsAsync(src))
            .then(hdus => {
                expect(hdus.length).toEqual(1);
    ...
    //     let src = new ArrayBufferFile(fs.readFileSync('data/SLR1_astrometry.fits'));
    //     return src.initialize()
    //         .then(_ => FitsReader.readFitsAsync(src))
    //         .then(hdus => {
    //             expect(hdus.length).toEqual(1);
    ...
    //     let src = new ArrayBufferFile(fs.readFileSync('data/1904-66_ZEA.fits'));
    //     return src.initialize()
    //         .then(_ => FitsReader.readFitsAsync(src))
    //         .then(hdus => {
    //             expect(hdus.length).toEqual(1);
    ...
    //             let val = converter.convert({ x: 0, y: 0 });
    //             expect(val.alpha).toBeCloseTo(17.926286462991737, 8);
    //             expect(val.delta).toBeCloseTo(-73.57489559932927, 8);
    //         });
    // });
