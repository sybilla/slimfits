...
export class CompressedImageReader implements IDataReader {
        ...
        return this.binaryTableDataReader.readDataAsync(file, offsetBytes, header)
            .then(result => {
                ...
                if (((bitpix === BitPix.Float32) || (bitpix === BitPix.Float64))
                    ...
                    const bytesPerElement = BitPixUtils.getByteSize(bitpix);
                ...
                const zscale = result.filter(column => column.name === 'ZSCALE')[0];
                const zzero = result.filter(column => column.name === 'ZZERO')[0];
        ...
        if (quantiz === 'NO_DITHER') {
            ...
            tiles.forEach((tile, tileNum) => {
                ...
                for (let i = 0; i < tile.length; i++) {
                    ...
                    if (i1 === 10000) {
                        i0 = (i0 + 1) % 10000;
                        i1 = parseInt(rn[i0] * 500 as any, 10);
                    }
                }
            });
        } else {
    ...
    private static initRandoms() {
        ...
        for (let i = 0; i < random.length; i++) {
            const temp = a * seed;
            seed = temp - m * parseInt((temp / m) as any, 10);
            random[i] = seed / m;
        }
